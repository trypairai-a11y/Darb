...
| Fleet portal | (own rail) | `/fleet-portal/*` — roster (+ `/drivers/[id]` profile), issues, documents, scorecard, payouts, support, team | FLEET role, fenced, per-tab |
...
- `/api/fleet/drivers` + `/documents` + `/issues` + `/support` — the fleet portal's request desk (revision 12). Submitting a driver, uploading a document, reporting leave or a resignation all open a `FleetChangeRequest`; **the only endpoint that writes a live record is `PATCH /api/fleet/drivers/:id/phone`**. Staff review at `/api/fleets/:id/requests/:reqId/approve|reject` (OPS_MANAGER+), and `GET /api/fleets/requests/pending-count` feeds the badge.
...
- **A granted tab must be able to appear** (revision 11 #9): Team and Settings carried `vendorRoles: ["OWNER"]` in `navConfig.ts` on top of their tab, and that gate runs first, so an owner who ticked Team for their accountant saw it save and nothing change. Rail entries are governed by the **tab list alone** now, and `GET /team` by `requireVendorTab("TEAM")` alone. What stays OWNER-only is the dangerous work on those screens — `POST/PATCH /team` and `POST /pause` — enforced on the endpoint AND by hiding the control (Add user, the active toggle, Edit access, the pause switch), so nobody is shown a button that answers 403.
- **A refused tab is locked, never a 403 screen** (revision 11 #7): `requireVendorTab` answers `{ error, code: "TAB_NOT_GRANTED", tab }`, the axios interceptor feeds that into `lib/vendorAccess.ts`, and the rail draws that entry greyed with a padlock while the vendor layout shows `AccessRestricted`. The rail no longer *filters* on tabs — it locks — so "where did Wallet go" has an answer on screen. The store only ever adds a restriction the server just enforced and clears on sign-out. **Any new 403 that means "not your tab" must carry that code**, or the portal is back to guessing from a URL. The role-based 403 beside it deliberately carries no code: a tracker refused the New order endpoint has not lost the Orders tab.
- **The shop may cancel through ARRIVED** (revision 11 #8): vendor cancel is pre-pickup, and `ARRIVED` is pre-pickup — the driver is at the counter and has not taken the bag, which is exactly when a kitchen discovers it cannot fulfil. Revision 8 (#3) inserted that status and left it out of the endpoint's `allowFrom`, so no surface could call one off. The button lives on the order board card as well as the detail page, gated client-side on the same OWNER/ORDER_TRACKING pair the endpoint enforces.
...
- **The fleet portal never writes a live record** (revision 12, `services/fleet/fleetRequestService.ts`): everything a delivery company wants is a `FleetChangeRequest`, and Darb's approval is what creates the driver, flips the status or marks the document valid. **A pending driver has NO `Driver` row** — approval creates it, which is what stops a rejected submission leaving a half-driver in the dispatch candidate pool. The roster draws pending drivers from the request itself so the ask is visibly somewhere. Approve/reject are status-guarded `updateMany` claims (count 0 = 409), same discipline as the order FSM. **The one exception is the driver's phone number**, which writes straight through at the client's request: drivers change SIMs constantly and a review queue between a driver and their own number strands them. The cost is real and deliberate, phone is the driver app's sign-in identity.
- **The fleet portal has its own tabs and roles** (revision 13 #6, `services/fleet/fleetTabService.ts` + `requireFleetTab`/`loadFleetIdentity` in `routes/fleetPortal.ts`, mirrored client-side in `frontend/src/lib/fleetTabs.ts`): role default (OWNER = everything; OPERATIONS = ROSTER/ISSUES/DOCUMENTS/SUPPORT; FINANCE = PAYOUTS/SCORECARD/SUPPORT), then a per-user `User.fleetTabs` JSON array that REPLACES it. `null` reads as OWNER and inherits, so this shipped with no backfill. Every rule the merchant portal learned applies here and is not re-litigated: **identity comes from the User row on every request, never the JWT**; **a refused tab answers `TAB_NOT_GRANTED` and draws a padlock**, never a 403 screen; **a granted tab must be able to appear**, so no second role gate sits on a tab in `navConfig` and what stays OWNER-only is the dangerous work (`POST/PATCH /api/fleet/team`), enforced on the endpoint AND by hiding the control. `User.fleetPartnerIds` is the fleet's answer to a vendor's `branchId`: it narrows an owner-group login to a subset of the group's companies, **intersected with the group in `fleetContext` and never unioned**, so a login scoped to Marina cannot reach Sidra by editing a query string.
- **A payout is not Darb's to process alone** (revision 13 #8): `FleetPayoutStatement.status` is `FINAL | CONFIRMED | DISPUTED | PAID`, the delivery company confirms or disputes from `/api/fleet/statements/:id/confirm|dispute`, and **`postFleetPayout` refuses anything that is not CONFIRMED** — the gate is in the service, not the route, so the cron and a script are bound by it too. A dispute needs a reason and opens a `SupportTicket` (type `WALLET`, labelled "Payout" on the fleet side only) carrying the period and the total, and clears any earlier confirmation. **Zero-total statements still flip to PAID unconfirmed**: there is nothing to disagree with about KD 0.000.
- **Confirming a payout means uploading the stamped invoice** (revision 13b, `services/fleet/fleetInvoiceService.ts` + `FleetPayoutInvoice`): `POST /api/fleet/statements/:id/confirm` refuses with `code: "INVOICE_REQUIRED"` unless the company's own stamped invoice is attached, and **the file is stored before the status moves** — a statement reading CONFIRMED with no invoice behind it is the outcome that endpoint must never leave. Re-confirming after a dispute upserts, so one month never carries two invoices. Staff read it at `GET /api/fleets/statements/:id/invoice`. Storage is R2 when configured and a `bytea` column (3MB cap, Vercel's body limit is 4.5MB and base64 adds a third) when it is not; `fileKey` is always preferred, so nothing needs migrating the day R2 is switched on. `GET /api/fleet/statements/:id` is the detail behind the confirmation: the orders the total was built from, plus the count the month was cut on beside the count that lists today, because a difference there is the company's to dispute rather than Darb's to silently reconcile.
- **The scorecard names people, not just averages** (revision 13b, `services/fleet/fleetDriverScoreService.ts`): `/api/fleet/scorecard` and `/api/fleets/:id/scorecard` return the same payload, now with six more KPIs and `drivers: { top, bottom }` — the three carrying the company against the three who are not, scored on-time 40 / acceptance 30 / rating 30. **A missing component is dropped and the weights re-normalised, never scored zero**, or every new joiner with no ratings lands in the bottom three by construction, and a driver needs 5 deliveries in the window to be ranked at all.
- **A driver status request carries its dates** (revision 13 #4/#5): LEAVE needs `leaveStartDate` and `returnDate` (return not before start), TERMINATED needs `lastWorkingDate`, validated on the endpoint as well as in the form. Approval still applies the status immediately — deferring it to a future date would need a scheduler, a reversal path and a story for an early return.
- **Approving a document writes through to the `Driver` columns** (`applyDriverDocToDriver`): `FleetDocument` is the file plus the audit trail, the `Driver.<doc>Expiry`/`<doc>Status` pairs stay the read model. That is what lets the roster's "3/4", the staff driver profile and the expiry alerts keep working untouched. A new driver document type MUST be added to `DRIVER_DOC_COLUMNS` or it is storable and invisible.
- **Fleet issues flow Darb → fleet; support flows fleet → Darb** (revision 12). Two tabs because they are two jobs; `SupportTicket` is one table (`vendorId` nullable, `fleetPartnerId` beside it, exactly one set) because Darb triages one inbox. `FleetIssue` is opened by the nightly sweep in `/api/cron/daily`, idempotent on `dedupeKey`, and **resolving requires a note**: an acknowledge button with no account of the fix is a button that gets clicked to clear a badge. Unresolved after 48h auto-escalates.
...
- **Document upload is not live yet.** The fleet portal signs direct-to-R2 PUTs via `services/r2Service.ts`, but `pair-darb-api` production has none of `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`. Until they are set, `POST /api/fleet/documents/upload-url` fails closed with 503 `{ code: "STORAGE_NOT_CONFIGURED" }`. The import button is **always on the panel** (revision 13 #2, `components/fleet/DocumentFileField.tsx`), disabled with the reason underneath rather than absent: hiding it is what made the client report that Add document had no way to attach a file. Switching it on also needs **a CORS rule on the bucket allowing PUT from `darbkw.vercel.app`** — the mobile app never hit that because React Native does not enforce CORS, and a browser does.
