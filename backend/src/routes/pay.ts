...
router.get("/:token", readLimiter, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      // Present only when a real gateway issued a checkout page. The Darb-hosted
      // page redirects to it; with no gateway configured this is null and the
      // page shows the transfer reference instead of a dead redirect.
