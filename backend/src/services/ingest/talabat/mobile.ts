// Phase 6 Wave 2b — TalabatMobileAdapter
//
// Reads MobileGPS-sourced records — LocationLog for `isAvailable`, OrderLog
// (`source: AGENT_CAPTURE`, `platform: TALABAT`) for `fetchOrders`. Both
// queries JOIN through `Driver.tenantId` (Pitfall 5) so a cross-tenant
// driver row can never surface in another tenant's view.
//
// Mirror of KeetaMobileAdapter — same shape, platform string changed to
// TALABAT. Phase 6 scope is fetchOrders only — shifts / attendance / cash /
...
export class TalabatMobileAdapter implements IngestAdapter {
  ...
  /**
   * AGENT_CAPTURE-sourced orders for Talabat in the requested window. Wave 5
   * will widen this to the new MOBILE_GPS enum value once the v0.6 mobile
   * build promotes everything to it.
   */
  async fetchOrders(
    tenantId: string,
    range: DateRange,
